...
export class ManageAbsComponent implements OnInit{
  ...
  displayedColumns: string[] = ['nom', 'type', 'dateStart', 'dateEnd', 'description','action'];
  ...
  onStatusSelect(): void {
    if (this.selectedStatus === 'approved') {
      this.displayedColumns = ['nom', 'type', 'dateStart', 'dateEnd', 'description'];
      this.fetchApprovedAbsences();
    } else if (this.selectedStatus === 'denied') {
      this.displayedColumns = ['nom', 'type', 'dateStart', 'dateEnd', 'description'];
      this.fetchDeniedAbsences();
    } else if (this.selectedStatus === 'pending') {
      this.displayedColumns = ['nom', 'type', 'dateStart', 'dateEnd', 'description', 'action'];
      this.fetchPendingAbsences();
    }
  }
